...
bump-patch:
	...
	@sed -i '' 's/__version__ = ".*"/__version__ = "$(shell echo $(VERSION) | awk -F. '{print $$1"."$$2"."$$3+1}')"/' coralogix_mcp/__init__.py

bump-minor:
	@echo "Bumping minor version..."
	@sed -i '' 's/version = ".*"/version = "$(shell echo $(VERSION) | awk -F. '{print $$1"."$$2+1".0"}')"/' pyproject.toml
	@sed -i '' 's/__version__ = ".*"/__version__ = "$(shell echo $(VERSION) | awk -F. '{print $$1"."$$2+1".0"}')"/' coralogix_mcp/__init__.py

bump-major:
	@echo "Bumping major version..."
	@sed -i '' 's/version = ".*"/version = "$(shell echo $(VERSION) | awk -F. '{print $$1+1".0.0"}')"/' pyproject.toml
	@sed -i '' 's/__version__ = ".*"/__version__ = "$(shell echo $(VERSION) | awk -F. '{print $$1+1".0.0"}')"/' coralogix_mcp/__init__.py 